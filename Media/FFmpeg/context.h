...
namespace av
{
    class format_context
    {
        ...
        std::shared_future<void> init_;     //CopyConstructible require
    public:
    ...
    packet format_context::read()
    {
        ...
        while(av_read_frame(handle_.get(),ptr(pkt))==0)
        {
            ...
            {   //assume single IO thread, TLS if parallelized
                static std::map<decltype(pkt->stream_index),AVMediaType> cache;
                ...
                if(pkt_type==static_cast<AVMediaType>(Media::value))
                    break;
                pkt.unref();
            }
        }
