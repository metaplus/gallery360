    ...
    , io_handle_(std::move(io))
{
    ...
    core::verify(avformat_open_input(&ptr, nullptr, av_find_input_format(iformat.name.data()), nullptr));
    format_handle_.reset(ptr, [](pointer p) { avformat_close_input(&p); });
    ...
    , io_handle_()
{
    ...
    pointer ptr = nullptr;
    core::verify(avformat_open_input(&ptr, ipath.url.data(), nullptr, nullptr));
    format_handle_.reset(ptr, [](pointer p) { avformat_close_input(&p); });
    core::verify(avformat_find_stream_info(ptr, nullptr));   // 60ms+
#ifdef _DEBUG
    av_dump_format(ptr, 0, ptr->filename, 0);
...
std::pair<av::codec, av::stream> av::format_context::demux_with_codec(const media::type media_type) const
{
    ...
    const auto ptr = format_handle_.get();
...
{
    packet pkt;
    while (av_read_frame(format_handle_.get(), get_pointer(pkt)) == 0
...
av::codec_context::codec_context(codec codec, const stream stream, const unsigned threads)
    : codec_handle_(avcodec_alloc_context3(get_pointer(codec)), [](pointer p) { avcodec_free_context(&p); })
    , stream_(stream)
    , state_()
{
    ...
    core::verify(avcodec_open2(codec_handle_.get(), get_pointer(codec), nullptr));
}
...
std::vector<av::frame> av::codec_context::decode(const packet& packets) const
{
    ...
    core::verify(avcodec_send_packet(get_pointer(codec_handle_), get_pointer(packets)));
    frame current;
    while (avcodec_receive_frame(get_pointer(codec_handle_), get_pointer(current)) == 0)
