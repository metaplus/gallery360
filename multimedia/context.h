...
namespace media
{
    class io_context
    {
        struct io_interface_internal
        {
            virtual ~io_interface_internal() = default;
        ...
        std::shared_ptr<io_interface_internal> io_interface_;
        std::shared_ptr<AVIOContext> io_handle_;

    public:
        using value_type = AVIOContext;
        using pointer = AVIOContext * ;
        using io_interface = io_interface_internal;
        ...
        explicit io_context(std::shared_ptr<io_interface_internal> io);
    ...
    public:
        ...
        format_context(format_context &&) = default;
        format_context& operator=(format_context const&) = default;
        format_context& operator=(format_context &&) = default;
        pointer operator->() const;
        explicit operator bool() const;
    ...
    public:
        ...
        codec_context(codec_context &&) = default;
        codec_context& operator=(codec_context const&) = default;
        codec_context& operator=(codec_context &&) = default;
        pointer operator->() const;
        explicit operator bool() const;
