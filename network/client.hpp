...
namespace net::client
{
    ...
    public:
        ...
        > async_send_request(std::string_view host, std::string_view target)
        {
            ...
            return async_send_request(std::move(request));
        }

        template<typename SendBody>
        boost::future<
            boost::beast::http::response<boost::beast::http::dynamic_body>
        > async_send_request(boost::beast::http::request<SendBody>&& request)
        {
            namespace http = boost::beast::http;
            static_assert(http::is_body<SendBody>::value);
            fmt::print("session: async_send_request\n");
            response_promise_ = {};
            auto request_ptr = std::make_unique<http::request<SendBody>>(std::move(request));
    ...
    private:
        ...
        folly::Function<void(boost::system::error_code, std::size_t) const>
            on_send_request(std::unique_ptr<boost::beast::http::request<SendBody>> request)
        {
            namespace http = boost::beast::http;
